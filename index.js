...
$(document).ready(function(){
  ...
  $("form.quiz").on("click", "button[type=submit]", function(event){
    ...
    if ($('input[name="answer-choices"]:checked').length) {
      ...
      alert("WHOOPS! You must select an answer before proceeding to the next question.");
    }
  })
  ...
  $(".feedback-page-correct, .feedback-page-incorrect").on("click", ".next-button", function(event){
...
function checkAnswer() {
  ...
  const userAnswer = $('input[name="answer-choices"]:checked').val().toString();
  ...
  if (userAnswer === correctAnswer) {
    ...
    $("p.solution").html("");
    $("p.solution").append("The correct answer was: " + bitcoinQuiz[viewedQuestionNum].choices[bitcoinQuiz[viewedQuestionNum].correct] + ".");
  }
...
function displayQandA() {
  ...
  for (i = 0; i < bitcoinQuiz[viewedQuestionNum].choices.length; i++) {
    $(".multiple-choices").append(`<p class="choice">
      ...
      <label for="answer-choice-${i}">${bitcoinQuiz[viewedQuestionNum].choices[i]}</label>
      </p>`);
  };
...
function updateViewedQuestionNumber() {
  ...
  $("legend.viewed-question-number").html(`Quiz Question #` +
    (viewedQuestionNum + 1) + ` of ` + bitcoinQuiz.length);
}
...
function updateScore() {
  ...
  $("p.current-score").html(`Your Score: ` + ` Correct = `
    + scoreCorrect + ` ; Incorrect = ` + scoreIncorrect);
}
...
function scoreSummary(){
  ...
  $("p.score-summary").html(`You got ` + scoreCorrect + ` out of ` + bitcoinQuiz.length + ` total questions correct.`);
  ...
  // is displayed.
  if ((scoreCorrect >= bitcoinQuiz.length - 3) && (scoreCorrect <= bitcoinQuiz.length - 1)) {
